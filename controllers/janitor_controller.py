...
def schedule():
    ...
    if request.method == 'GET':
         req = request.args.to_dict()
         if 'schedule_janitor' in req:
             data["assigned"] = dbms.selectScheduleInDate(req["schedule_janitor"])

    content = render_template('components/schedule_janitor.html', data = data)
